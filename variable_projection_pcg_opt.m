function runhist = variable_projection_pcg_opt(U0,V0,MA,vp_paras,cg_paras,paras)
    timect = cputime;
    m = paras.m;
    n = paras.n;
    r = paras.r;
    ...
    fprintf("variable_projection_pcg_opt method, the initial objective value fval is %3.4f\n",fval(U,V,MA));
    ...
    while (iter < itmax && norm(gra(:))>tol)
        ...
        tic
        D1P1_opt=compute_kron_blocked(P1,tildeV_1);
        toc
        DP_res=D1P1-D1P1_opt;

        fprintf("-----------the residual of the D1P1 is %3.40f------------\n",norm(DP_res(:)));
        ...
        Coff=A1TA1+A2TA2+A3TA3+0.00001*eye((m+n)*r);

        %% cg method
        runhist = pcg(Coff,B0(:),cg_paras);
       ...
       %% check whether [A;B] is the descent direction
        ...
        fprintf("variable_projection_pcg_opt %d-th iteration, fval is %3.20f =============================================================\n",iter,fval(U,V,MA_temp));
