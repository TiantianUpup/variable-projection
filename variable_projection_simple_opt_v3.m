function runhist = variable_projection_simple_opt_v3(U0,V0,U_true,V_true,W_true,MA,vp_paras,cg_paras,paras)
    timect = cputime;
    m = paras.m;
    n = paras.n;
    r = paras.r;
    ...
    fprintf("=========================== fval init opt gap is ==================================%3.20f\n",fval_tilde_opt(WTW,P1TU,Q1TV,W1)-fval_tilde_opt_v3(WTW,N));
    % fprintf("--------------------------- fval init opt gap is ----------------------------------%3.20f\n",fval_init_opt-fval_init);
    ...
    while (iter < itmax)
        ...
        %%%%%%%%%%%%%%%%%% the following code calculates B (opt method)
        %tic
        Z1=zeros(r,r);
        Z2=zeros(m-r,r);
        Z3=zeros(r,r);
        ...
        for i=1:r
            ...
            Btemp_1=WTWNDiFi.*Q1TVvi;
            Btemp_2=WTWNDiFi.*P1TUui;

            a_mat=reshape(W1NTWTWNDiFi,[r,r]);
            Z1(:,i)=P1TU*Btemp_1-a_mat'*tildeV_1_i;
            Z2(:,i)=P2TU*Btemp_1;
            Z3(:,i)=Q1TV*Btemp_2-a_mat*tildeU_1_i;
            Z4(:,i)=Q2TV*Btemp_2;
        end
        ...
        runhist = cg_subproblem_simple_v3(Aparas,cg_paras,paras,B0,gamma);
        ...
        for i=1:150
            ...
            fval_cur=fval_tilde_opt_v3(WTW,N)+0.5*MA_norm;
