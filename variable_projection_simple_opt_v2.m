function runhist = variable_projection_simple_opt_v2(U0,V0,U_true,V_true,W_true,MA,vp_paras,cg_paras,paras)
    timect = cputime;
    m = paras.m;
    n = paras.n;
    r = paras.r;
    ...
    % MA_norm_res=MA_norm-MA_norm_opt;
    % fprintf("================================ the residual of MA_norm is %3.20f==============================\n",MA_norm_res);
    
    fval_init=fval_tilde_opt(WTW,P1TU,Q1TV,W1)+0.5*MA_norm;
    %fprintf("---------------------- the calculateion of the fval_init costs %3.10f ----------------\n",cputime-cput_finit);
    fval_pre=fval_init;
    ...
    while (iter < itmax)
        ...
        % H1TMAC=zeros(r*r,r);
        % H22TMAC=zeros((m-r)*r,r);
        % H21TMAC=zeros((n-r)*r,r);
        % for i=1:r
        %     Mi=W1DiF(:,i);
        %     Mi_mat=reshape(Mi,[r,r]);
        %     beta_temp=sum((Mi_mat*P1TU).*Q1TV,1);
        %     beta=beta_temp';
        %     WTWbeta=WTW*beta;
        %     H1TMAC(:,i)=reshape(Q1TV.*WTWbeta'*P1TU',[r*r,1]);    
        %     H22TMAC(:,i)=reshape(Q1TV.*WTWbeta'*P2TU',[(m-r)*r,1]); 
        %     H21TMAC(:,i)=reshape(Q2TV.*WTWbeta'*P1TU',[(n-r)*r,1]); 
        % end  

        % W2H1TMAC=W2W2T*H1TMAC;
        ...
        %     Temp1_mat=reshape(W2H1TMAC(:,i),[r,r]);
        %     Temp2_mat=reshape(H22TMAC(:,i),[r,m-r]);

        %     Z1(:,i)=Temp1_mat'*tildeV_1_i;
        %     Z2(:,i)=Temp2_mat'*tildeV_1_i;
        %     Z3(:,i)=Temp1_mat*tildeU_1_i;
        %     Z4(:,i)=reshape(H21TMAC(:,i),[n-r,r])*tildeU_1_i;
        % end   
        ...
        for i=1:r
            ...
            H1TMACi=reshape(Q1TV.*WTWbeta'*P1TU',[r*r,1]);
            H1TMAC(:,i)=H1TMACi;
            W2H1TMACi=H1TMACi-W1*(W1'*H1TMACi);
            % W2H1TMACi=W2W2T*H1TMACi;

            Temp1_mat=reshape(W2H1TMACi,[r,r]);
            Temp2_mat=reshape(reshape(Q1TV.*WTWbeta'*P2TU',[(m-r)*r,1]),[r,m-r]);

            Z1(:,i)=Temp1_mat'*tildeV_1_i;
            Z2(:,i)=Temp2_mat'*tildeV_1_i;
            Z3(:,i)=Temp1_mat*tildeU_1_i;
            Z4(:,i)=reshape(reshape(Q2TV.*WTWbeta'*P1TU',[(n-r)*r,1]),[n-r,r])*tildeU_1_i;
        end   
 
 
        B0=[Z1;Z2;Z3;Z4];

        %fprintf("------------------------ the calculation of the B costs %3.20f -----------------------------\n", cputime-cput_start);
        % Z1=zeros(r,r);
        % Z2=zeros(m-r,r);
        % Z3=zeros(r,r);
        % Z4=zeros(n-r,r);
        ...
        % B0_simple=[Z1;Z2;Z3;Z4];


        % B_res=B0-B0_simple;
        % fprintf("========================== the residual of B is %3.20f =======================\n",norm(B_res)); 

        Aparas.H1TMAC=H1TMAC;
        Aparas.W1DiF=W1DiF;
        Aparas.W2W2T=W2W2T;
        Aparas.W1=W1;
        ...
        runhist = cg_subproblem_simple_v2(Aparas,cg_paras,paras,B0,gamma);
        ...
        for i=1:150
            ...
            fval_cur=fval_tilde_opt(WTW,P1TU,Q1TV,W1);
            f_diff=fval_pre-fval_cur;
