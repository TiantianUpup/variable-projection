function runhist = variable_projection_simple_opt(U0,V0,U_true,V_true,W_true,MA,vp_paras,cg_paras,paras)
    timect = cputime;
    m = paras.m;
    n = paras.n;
    r = paras.r;
    ...
    while (iter < itmax)
        ...
        for i=1:r
            Mi=M(i,:);
            Mi_mat=reshape(Mi,[n,m]);
            beta_temp=sum((Mi_mat*PTU).*QTV,1);
            beta=beta_temp';
            WTWbeta=WTW*beta;
            MAC(:,i)=reshape(QTV.*WTWbeta'*PTU',[m*n,1]);    
        end
