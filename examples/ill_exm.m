...
% truth factor matrices
    ...
    % U_true = 1e6*U_true / norm(U_true,'fro');          % ||U||=1
    % V_true = 1e-1 * V_true / norm(V_true,'fro');    % ||V||=1000
    % W_true = 1e2 * W_true / norm(W_true,'fro');   % ||W||=0.001
    ...
    % V_true = rand(n,r_true);
    % W_true = rand(p,r_true);
    % alpha = logspace(-6,6,r_true);
    % beta  = logspace(6,-6,r_true);

    % for k=1:r_true
    %     U_true(:,k)=alpha(k)*U_true(:,k);
    %     V_true(:,k)=beta(k)*V_true(:,k);
    %     W_true(:,k)=W_true(:,k)/(alpha(k)*beta(k));
    % end
